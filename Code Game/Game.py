...
class Game(object):
    ...
    equipments = {0: [Equipment("potion", "!", use=lambda self, hero: heal(hero,random.randint(2,5))),
                      ...
                      Amulette(name="AmuHeal", abbrv="h", effect={'hpMax': 5}, use=lambda self, hero: boost(hero,self.effect)),
                      Forgeron()],
    
                  1: [Equipment("potion", "!", use=lambda self, hero: teleport(hero, True)),
                  ...
                  2: [Arme(name="arc", effect={'strength': 2}, use=lambda self, hero: hero.throw(5, 2,"normal",4)),
                      Arme(name="halbarde", abbrv="|", effect={'strength': 3}, use=lambda self, hero: boost(hero,self.effect)),
    ...
    """Liste des boss"""
    boss = {0: [Boss("Thanos")]}
#Boss("Robin",categorie="distance",rangeAtk=4, elementAtk="poison", timeEffect=4,proba=90),Boss("Ragnar"),Boss("Joker"),Boss("Radahn"),Boss("Grievous"),
    ...
    def selectPerso(self):
        ...
        perso = [
        Hero(name="SwordMan", hp=20, abbrv="@", strength=4, size=5, txt=" @ : vous etes un epéiste de talent. votre point fort ? l'attaque ! Vous commencez avec une épée, amoureux de votre lame vous vous balader toujours avec de quoi la réparer", inventory=[Arme(name="épée", effect={'strength': 2}, use=lambda self, hero: boost(hero,self.effect))],acier=1),
        Hero(name="Tank", hp=30, abbrv="T", strength=2, size= 3, vision=5, txt=" T : Vous etes un homme énorme qui sait encaisser des dégats. vous commencez avec une armure, vous êtes borgne suite à vos combats, votre vision est plus faible", inventory=[Wearable(name="armure de bronze",abbrv="ab",place='corps', effect={'defense': 1}, use=lambda self,hero: boost(hero,self.effect))]),
        Hero(name="Archer", hp=15, abbrv="R", strength=2, size=5, vision=8, txt=" R : vous etes une archere qui fait pas mal de dégats et qui commence avec un arc, vos talents d'archer vous procure une vue accrue", inventory=[Arme(name="arc", effect={'strength': 2}, use=lambda self, hero: hero.throw(5, 2,"normal",4))],argent=1),
        Hero(name="LuckyGirl", hp=20, abbrv="L", strength=2, size=8, txt=" L : Vous etes une femme chanceuse qui commence avec 3 gold et deux objets aléatoires",inventory=[self.randEquipment(),self.randEquipment()],argent=3, acier=1)]
