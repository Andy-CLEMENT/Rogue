...
def tir(coordUser, dir, rangeAtk, strength, elementAtk,timeEffect,proba):
    ...
    if coordUser+dir in stage:            #si attaque à distance
        ...
        if cible==stage.ground:
            ...
            i=0
            while i<rangeAtk-1 and stage.pos(p)+dir in stage and (cible==stage.ground or hasattr(cible,"hp")):            #on effectue les déplacements du projectile jusqu'a ce qu'il atteingne sa cible ou sa portée
                a=stage.move(p,dir)
        ...
        elif hasattr(cible,"hp"):             #si attaque au cac contre une creature ou un hero
            m=cible.meet(p)
